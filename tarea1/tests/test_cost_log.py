...
def reg(ruta, **kw):
    base = dict(timestamp="2026-09-21T10:00:00-05:00", modelo="claude-haiku-4-5-20251001", tokens_in=2000, tokens_out=300,
                latencia_ms=850.4, costo_usd=0.0035, exito=True)
...
def test_una_linea_por_llamada_con_todos_los_campos(tmp_path):
    ...
    reg(ruta)
    reg(ruta, exito=False, error="rate limit", tokens_in=0, tokens_out=0, costo_usd=0.0)
...
def test_nunca_se_escribe_una_clave(tmp_path):
    ...
    ruta = tmp_path / "l.jsonl"
    reg(ruta, exito=False, error=f"401 con la clave {clave_a} y el token {clave_t}, Bearer abc123def456")
    contenido = ruta.read_text(encoding="utf-8")
    assert clave_a not in contenido and clave_t not in contenido and "abc123def456" not in contenido and "[CLAVE-OCULTA]" in contenido
...
def test_resumen_de_costos(tmp_path):
    ruta = tmp_path / "l.jsonl"
    reg(ruta, costo_usd=0.002, latencia_ms=800)
    reg(ruta, costo_usd=0.004, latencia_ms=1200)
    reg(ruta, exito=False, error="x", tokens_in=0, tokens_out=0, costo_usd=0.0, latencia_ms=50)
    r = resumen(leer_registros(ruta))
    assert (r["llamadas"], r["exitosas"], r["fallidas"]) == (3, 2, 1)
    assert r["costo_total_usd"] == 0.006 and r["costo_medio_por_consulta_usd"] == 0.003 and r["latencia_mediana_ms"] == 1000.0
    assert resumen([])["costo_medio_por_consulta_usd"] is None
