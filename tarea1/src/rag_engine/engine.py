...
Orden de las decisiones (importa):
  ...
  5. Todo error (índice, red, clave, cuota, formato) se devuelve en ``error``, con ``respuesta = None``: jamás como una respuesta normal.
...
from rag_engine.llm.anthropic_client import ClienteAnthropic, ErrorLLM, RespuestaLLM
...
class MotorRAG:
    def __init__(self, cfg: Config, embedder: Embedder, coleccion, modificaciones: Modificaciones, tabla_precios: TablaPrecios,
                 cliente_llm=None, ruta_log: Path | None = None, reloj=_ahora):
        ...
        self._herramienta = ClienteAnthropic.herramienta(cfg.get("prompts.herramienta_nombre"), cfg.get("prompts.herramienta_descripcion"),
                                                         cfg.get("prompts.herramienta_campos"))
    ...
    def desde_config(cls, cfg: Config | None = None) -> "MotorRAG":
        ...
        return cls(cfg, crear_embedder(cfg), abrir_para_lectura(cfg), Modificaciones.cargar(ruta_mod), cargar_tabla(cfg.ruta("pricing")))

    # ── piezas ──

    def _llm(self) -> ClienteAnthropic:
        """Se crea al primer uso: una abstención por umbral no necesita clave de API."""
        if self._cliente_llm is None:
            try:
                clave = self.cfg.requerir_env("ANTHROPIC_API_KEY", para="generar respuestas")
            except ConfigError as exc:
                raise ErrorLLM("autenticacion", str(exc), solicitud_enviada=False) from exc
            self._cliente_llm = ClienteAnthropic(self.cfg.get("llm"), api_key=clave)
    ...
    def _resultado_error(self, mensaje: str, t0: float, ts: str, fuentes: list[Fuente] | None = None, mejor: float = 0.0, modelo: str | None = None) -> ResultadoRAG:
        return ResultadoRAG(respuesta=None, fuentes=fuentes or [], abstuvo=False, mejor_similitud=mejor, latencia_ms=round((time.perf_counter() - t0) * 1000, 1),
                            modelo=modelo, error=mensaje, timestamp=ts)
    ...
    def responder(self, pregunta: str) -> ResultadoRAG:
        ...
        if not pregunta or not pregunta.strip():
            return self._resultado_error("La pregunta está vacía.", t0, ts)
        try:
            recuperados = buscar(self.coleccion, self.embedder, pregunta, self.cfg.get("retrieval.top_k"))
        except Exception as exc:
            return self._resultado_error(f"No se pudo consultar el índice: {exc}", t0, ts)
        ...
        # 4. generación
        modelo = self.cfg.get("llm.modelo")
        try:
            llm = self._llm()
            r: RespuestaLLM = llm.generar(self.cfg.get("prompts.sistema"), self._prompt_usuario(pregunta, fuentes), self._herramienta)
            ...
            if exc.solicitud_enviada:              # solo las llamadas que realmente salieron al proveedor van al log de costos
                registrar_llamada(self.ruta_log, timestamp=ts, modelo=modelo, tokens_in=0, tokens_out=0, latencia_ms=0.0, costo_usd=0.0, exito=False,
                                  error=f"{exc.tipo}: {exc.mensaje}")
            return self._resultado_error(exc.mensaje, t0, ts, fuentes, round(mejor, 4), modelo)
        try:
            costo = self.tabla.costo(r.modelo, r.tokens_in, r.tokens_out, r.momento)
        except ErrorPrecio as exc:
            registrar_llamada(self.ruta_log, timestamp=ts, modelo=r.modelo, tokens_in=r.tokens_in, tokens_out=r.tokens_out, latencia_ms=r.latencia_ms,
                              costo_usd=0.0, exito=False, error=f"precio: {exc}")
            return self._resultado_error(f"La respuesta se generó pero no se pudo calcular su costo: {exc}", t0, ts, fuentes, round(mejor, 4), r.modelo)
        registrar_llamada(self.ruta_log, timestamp=ts, modelo=r.modelo, tokens_in=r.tokens_in, tokens_out=r.tokens_out, latencia_ms=r.latencia_ms,
                          costo_usd=costo, exito=True)
        self._marcar_citadas(fuentes, r.citas)
        comunes = dict(fuentes=fuentes, mejor_similitud=round(mejor, 4), tokens_entrada=r.tokens_in, tokens_salida=r.tokens_out, costo_usd=costo,
                       latencia_ms=round((time.perf_counter() - t0) * 1000, 1), modelo=r.modelo, advertencias_version=advertencias, timestamp=ts)
...
def responder(pregunta: str, cfg: Config | None = None) -> ResultadoRAG:
    ...
    try:
        ...
        return ResultadoRAG(respuesta=None, error=str(exc), latencia_ms=round((time.perf_counter() - t0) * 1000, 1), timestamp=ts)
